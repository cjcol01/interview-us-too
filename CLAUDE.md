...
| `ADMIN_USERNAME` | No | Username whose session cookie `_require_author` trusts as admin, and the Basic-auth username paired with `AUTHOR_PASSWORD`. Deliberately not hardcoded — the repo is public. Unset, both paths fail closed and `/admin`, `/verify-author`, `/partner/admin`, `/admin/usage` are unreachable, and the navbar's Admin dropdown (`is_admin` in `_template_globals`) never renders. `run_tests.py` pins it to a test-only value so the suite never depends on the real one. |
| `SKIP_EMAIL_VERIFICATION` | No | Dev-only convenience flag — see `.env` for details. Must never be set in production. |
| `WEBSTORE_EXTENSION_ID` | No | 32-char Chrome Web Store item ID of the published extension. Set it and the listing is polled every 30 min (`_check_webstore` in `server.py`, via Chrome's own CRX update endpoint); an unpublish/takedown raises a danger alert on `/admin`, shows on `/admin/health`, and emails `NOTIFY_EMAIL` once per outage (needs two consecutive failed readings to confirm). Unset, that row reads "not configured" and the poll loop no-ops. |
...
| `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` | No | Enables "Continue with GitHub" (`/auth/github`, `/auth/github/callback` in `server.py`). Unset, the button is hidden and the routes 404. Callback URL to register on the GitHub OAuth App: `{BASE_URL}/auth/github/callback`. Requires the `user:email` scope (requested automatically) since GitHub only returns a user's email directly when it's public. |
