...
- **No health check / auto-restart monitoring beyond systemd.** `systemctl restart interviewace` handles crashes, but nothing alerts you when it happens or when the process is unhealthy but still "running" (e.g. DB locked, Redis down). A simple uptime/health-check ping (e.g. UptimeRobot hitting `/`) would close this gap cheaply.
